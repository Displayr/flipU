...
{
    if (is.null(names))
        StopUserError("There are no names to select from.")
    frm <- match(from, names)
    if (is.na(frm))
        StopUserError("'", from, "' is not one of the names.")
    t <- match(to, names)
    if (is.na(t))
        StopUserError(to, " is not one of the names.")
    frm:t
}
...
{
    if (is.null(MARGIN))
        StopUserError("'MARGIN needs to be specified. A 1 for rows and 2 for columns.")
...
{
    if (is.null(MARGIN))
        StopUserError("'MARGIN needs to be specified.")
    dims <- dim(x)
    if (!MARGIN %in% seq_len(length(dims)))
        StopUserError("'MARGIN' is invalid (not compatible with dimensions of the array).")
